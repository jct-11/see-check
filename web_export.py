...
def export_web_data(predictions, images_cpu, output_dir, conf_threshold=0.5):
    ...
    # ── 1. cameras.json ─────────────────────────────────────────────────
    # Export c2w R/t (camera-to-world, directly usable for visualization)
    cam_list = []
    for i in range(S):
        R_c2w = ext[i, :3, :3].tolist()
        t_c2w = ext[i, :3, 3].tolist()
        cam_list.append({
    ...
    # ── 2. metadata.json ────────────────────────────────────────────────
    # scene center from c2w camera positions
    cam_positions = ext[:, :3, 3]
